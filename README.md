...
Como demonstrado em grafico01_cobertura_geral.png, mesmo após o merge ainda existem alguns problemas com o dataset. Em especial, ainda existem muitos dados faltantes, especialmente dados da tabela do IHME. Remover todas as instâncias que possuem algum tipo de dado faltante poderia impactar diretamente na qualidade do conhecimento gerado, visto que mais da metade das instâncias teriam que ser removidas. Portanto, esses dados faltantes precisaram ser inferidos. 
Para isso, escolhemos a técnica da Interpolação Linear, na qual os dados declarados de um país específico serão usados para criar uma reta entre dois pontos conhecidos e os dados faltantes serão as projeções nessa reta. Contudo, é comum no dataset encontrar dados a partir de 1993 faltantes (começo do dataset), impedindo que se crie uma reta para estimar o valor de 1994, por exemplo (porque não existe nem 1993, nem 1994). Para isso, foi usada a técnica de Backfilling/ForwardFilling, na qual o primeiro valor válido encontrado é propagado horizontalmente até o ínicio ou fim da série. Assim, suponhamos que o primeiro registro de um país seja em 1996, dessa forma, os dados de 1993 até 1995 serão os mesmos de 1996. O mesmo acontece para dados faltantes no fim. 
A escolha de cada um desses métodos foi cautelosa e diretamente relacionada ao problema. Utilizar o Backfilling/Forwardfilling é melhor que estimar os valores com os outros pontos para os valores das bordas pois reduz o risco de se criar uma tendência irreal, reduz a amplificação de erros e, principalmente, não cria possíveis relações artificais. Como existem muitos casos em que os dados só são registrados a partir de 2000, por exemplo, muitos dados teriam que ser inferidos até 2000, podendo criar uma relação artificial entre dois atributos que se propagaria por muitos anos e impactar diretamente o aprendizado do modelo (Random Forest) futuramente. Além disso, no contexto do problema, variáveis socioeconômicas não costumam seguir retas infinitas, mas sim ciclos, o que faz do Backfilling/Forwardfilling a abordagem mais conservadora para esse problema.
Além disso, o uso da Interpolação Linear no lugar de outra interpolação que crie curvas suaves (como LaGrange ou Spines) é preferível na economia para evitar o 'Fenômeno de Runge', na qual a curva começa a oscilar violentamente em suas extremidades.
Esses métodos possuem uma principal limitação, ele estima que não houveram variações bruscas nso dados de um ano para outro (o que pode acontecer em um cenário de guerra ou pandemia, por exemplo).
No geral, as escolhas desses métodos foram visando abordagens conservadoras, que menos impactem futuramente o modelo e que façam sentido no contexto do problema (atributos socioeconômicos), sem precisar descartar os dados.

O segundo problema era a discrepância muito grande entre os valores mundiais do PIB. Por mais que a normalização não seja necessariamente obrigatória para o algoritmo de Random Forest, sem a aplicação do Log o algoritmo poderia gastar muito esforço tentando criar divisões nos valores gigantes (outliers) e ignorar nuances importantes entre os países de renda média e baixa. O Log "achata" essa disparidade, distribuindo os países de forma mais equilibrada. Isso ajuda o Random Forest a encontrar pontos de corte (splits) muito mais inteligentes.

Como uma árvore de decisão decide onde cortar um dado baseando-se apenas na ordem dos valores (por exemplo, PIB > 1000?), não importa se os dados estão ou não normalizados, o ponte de corte ideal será sempre proporcionalmente o mesmo. Contudo, caso seja de interesse futuramente de se comparar os resultados com outro modelo, os dados precisarão estar obrigatoriamente normalizados. Além disso, normalizá-los não irá gerar nenhum tipo de perda de conhecimento. Dessa forma, foi aplicada uma normalização Z-score em todos os dados numéricos exceto no ano.