...
# Validação cruzada com GroupKFold (países inteiros em cada fold)
print(f"Validação cruzada (GroupKFold, k={N_SPLITS})")
print(f"Todos os anos de um mesmo país ficam no mesmo fold.")
...
for target in TARGET_COLS:
    ...
    r2_scores   = cross_val_score(rf, X, y, cv=gkf, groups=groups, scoring='r2')
    mae_scores  = -cross_val_score(rf, X, y, cv=gkf, groups=groups, scoring='neg_mean_absolute_error')
    rmse_scores = np.sqrt(-cross_val_score(rf, X, y, cv=gkf, groups=groups, scoring='neg_mean_squared_error'))
