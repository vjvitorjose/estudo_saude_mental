...
for target in TARGET_COLS:
    ...
    r2_scores   = cross_val_score(xgb, X, y, cv=cv_folds, scoring='r2')
    mae_scores  = -cross_val_score(xgb, X, y, cv=cv_folds, scoring='neg_mean_absolute_error')
    rmse_scores = np.sqrt(-cross_val_score(xgb, X, y, cv=cv_folds, scoring='neg_mean_squared_error'))
...
for target in TARGET_COLS:
    ...
    # Importância por ganho (equivalente ao MDI no Random Forest)
    importances_gain[target] = xgb.feature_importances_

    perm = permutation_importance(xgb, X, y, n_repeats=20, random_state=42, n_jobs=-1)
...
plt.tight_layout()
plt.savefig(PLOTS_DIR / 'xgb01_metricas_por_transtorno.png', dpi=150)
plt.close()
print("\nxgb01_metricas_por_transtorno.png")

# Gráfico de importância por ganho por transtorno
fig, axes = plt.subplots(1, len(TARGET_COLS), figsize=(18, 5))
fig.suptitle('Importância das features (ganho) — XGBoost', fontsize=12)

for ax, target in zip(axes, TARGET_COLS):
    imp   = importances_gain[target]
...
plt.tight_layout()
plt.savefig(PLOTS_DIR / 'xgb02_importancia_ganho.png', dpi=150)
plt.close()
print("xgb02_importancia_ganho.png")
...
ax.set_title('Importância por permutação — XGBoost\n(média sobre 20 repetições)', fontsize=12)
ax.set_xlabel('Transtorno')
ax.set_ylabel('Feature')
plt.tight_layout()
plt.savefig(PLOTS_DIR / 'xgb03_importancia_permutacao.png', dpi=150)
plt.close()
print("xgb03_importancia_permutacao.png")
